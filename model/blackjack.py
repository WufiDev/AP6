...
class Baraja:
    ...
    def repartir_carta(self, tapada: bool) -> Optional[Carta]:
...
class Blackjack:
    ...
    def iniciar_juego(self, apuesta: int, revolver: bool):
        if not self.jugador:
            return "Por favor, regsitra un usuario primero"
        
        if not self.jugador.tiene_fichas():
            return "No tiene fichas para apostar."
        
        if apuesta <=0:
            return "La puesta debe ser mayor que cero"
        
        if not self.jugador.puede_apostar(apuesta):
            return "No tienes suficientes fichas para esta apuesta"
        ...
        return f"Comienza el juego. Tu apuesta actual es: {apuesta} fichas."
